...
void c2f(float *out, unsigned char byte) {
  out[3] = b2f(byte & 3); // bits 0-1
  out[2] = b2f((byte >> 2) & 3); // bits 2-3
  out[1] = b2f((byte >> 4) & 3); // bits 4-5
  out[0] = b2f((byte >> 6) & 3); // bits 6-7
}
...
/**
 * Return 0 for keep processing instructions, 1 for done.
*/
int ins(char **code, char delimit, float args[8]) {
  char opcode = *((*code)++);

  // terminate once we reach the delimiter as an opcode
  if (opcode == delimit) {
    return 1;
  }
  ...
  if (isalnum(opcode) && opcode != 'm' && opcode != 'M' && opcode != 'A' &&
    ...
    // lookahead for '='
    char eq = *((*code)++);
    if (eq != '=') { // push arg or variable 
      (*code)--;

      // numbers >=2 correspond to arguments (2 is first arg)
      if (!isalpha(opcode)) {
        push(args[opcode - '2']); // char_code - 50
      } 
      // letters other than m, M, A correspond to variables
      else {
        push(vars[opcode]);
      }
    } else {
  ...
  if (opcode == ':') {
    ...
    // skip chars until we find next ':'
    while (**code != ':') {
      (*code)++;
    }

    **code = 0; // cut off previous function?
    (*code)++;

    fdatas[funcid] = *code; // store function body, will get cut off later

    // keep advancing till end of function
    int nest = 1;
    while (nest) {
      char ch = *((*code)++);

      // '=' means execute more instructions?
      if (ch == '=')
        nest++;
  ...
  // run function
  if (opcode == '^') {
    // find which function we are calling
    for (int i = 0; i < nfs; i++) {

      // search for next few chars in function names table
      if (strstr(*code, fnames[i]) == *code) {
        char *fcode = fdatas[i];

        // read args from top of stack
        float newargs[8];
        for (int j = 0; j < 8; j++) {
          if (j < sp) {
...
int encrypt_byte(char *flagbyte, unsigned int chain, int idx,
  ...
  // store input bytes as float args
  ...
  // creates variables a-q with bytes from chain
  for (int i = 0; i < 4; i++) {
    c2f(temp, chain & 0xff);
    printf("%d\n", chain & 0xff);
  ...
  // run alg to encrypt byte
  while (!ins(&codep, ';', inargs))
    ;

  // load checksum and encrypted byte off stack
  *checksum = cpop();
  *(flagbyte + 1) = cpop();
  *(flagbyte) = cpop();

  // load chain back off stack
  chain = cpop();
  chain = (chain << 8) | cpop();
  chain = (chain << 8) | cpop();
  chain = (chain << 8) | cpop();
...
int main() {
  ...
  // initial run to setup functions
  while (!ins(&codep, ';', inargs))
    ;
  ...
  // encrypt the flag in 2 char chunks
